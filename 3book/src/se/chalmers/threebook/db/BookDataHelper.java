...
public class BookDataHelper {
	...
	public static List<Book> getRecentBooks(Context context) {
		return getBooks(context, null, BookTable.COLUMN_LASTREAD + " IS NOT NULL", null, BookTable.COLUMN_LASTREAD);
	}
	...
	private static List<Book> getBooks(Context context, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		for (bookCursor.moveToFirst(); !bookCursor.isAfterLast(); bookCursor.moveToNext()) {
			Book book = new Book()
				...
				.setTitle(bookCursor.getString(bookCursor.getColumnIndex(BookTable.COLUMN_TITLE)))
				.setSource(bookCursor.getString(bookCursor.getColumnIndex(BookTable.COLUMN_SOURCE)));
